...
class PIDLateralControl():
    ...
    def pid_controller(self, waypoint, vehicle_transform):
        ...
        v_vec = np.array([v_end.x - v_begin.x, v_end.y - v_begin.y, 0.0]) # 향하고 있는 방향벡터
        w_vec = np.array([waypoint.transform.location.x - v_begin.x, waypoint.transform.location.y - v_begin.y,0.0]) #목표와 현재 빼기
        ...
        if len(self.errorBuffer)>=2:
            ...
            de = 0.0
            ie = 0.0
        print("steering",np.clip((self.K_P*dot)+(self.K_I*ie)+(self.K_D*de), -1.0,1.0))
...
def main():
    ...
    try:
        ...
        while True:
            ...
            #PIDLateralControl.pid_controller(waypoint,vehicle.get_transform())
            # camera_bp = blueprint_library.find('sensor.camera.semantic_segmentation')
            # camera_bp.set_attribute('image_size_x','800')
            # camera_bp.set_attribute('image_size_y', '600')
            # camera_bp.set_attribute('fov', '90')
            # camera_transform = carla.Transform(carla.Location(x=100,y=200))
            # camera = world.spawn_actor(camera_bp, camera_transform)
            # camera.listen(lambda image : image.save_to_disk('output{}'.format(image.frame), carla.ColorConverter.CityScapesPalette))
            #
            # depth_camera_bp = blueprint_library.find('sensor.camera.depth')
            #
            # depth_camera_transform = carla.Transform(carla.Location(x=100, y=200))
            # depth_camera = world.spawn_actor(depth_camera_bp, depth_camera_transform)
            # depth_camera.listen(lambda image: image.save_to_disk('output{}'.format(image.frame), carla.ColorConverter.LogarithmicDepth))
